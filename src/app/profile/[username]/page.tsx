  ...
  return (
    <div className={`min-h-screen ${experience.pageClass} ${experience.fontClass}`}>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-20 pt-6 sm:px-6 lg:px-8">
        ...
        ) : experience.key === "earthskydesign" ? (
          <section className={`overflow-hidden rounded-[2.3rem] ${experience.panelClass}`}>
            <div className="grid gap-0 lg:grid-cols-[1.08fr_0.92fr]">
              <div className="p-8 sm:p-10">
                <p className={`text-xs font-semibold uppercase tracking-[0.28em] ${experience.mutedClass}`}>Organic horizon profile</p>
                <h1 className="mt-5 text-4xl font-semibold tracking-[-0.05em] text-foreground sm:text-5xl">{brandName}</h1>
                {domain ? (
                  <div className={`mt-5 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm ${experience.softPanelClass}`}>
                    <Globe className="h-4 w-4" />
                    {domain}
                  </div>
                ) : null}
                              </div>
              <div className="border-t border-teal-100 bg-[linear-gradient(180deg,rgba(204,251,241,0.38),rgba(255,255,255,0.82))] p-6 sm:p-8 lg:border-l lg:border-t-0">
                <div className="grid gap-5">
                  <div className="relative aspect-[16/10] overflow-hidden rounded-[2rem] bg-muted">
                    {coverUrl ? <ContentImage src={coverUrl} alt={`${brandName} cover`} fill className="object-cover" /> : null}
                  </div>
                  <div className="flex items-center gap-4 rounded-[1.6rem] border border-teal-100 bg-white/80 p-4">
                    <div className="relative h-20 w-20 overflow-hidden rounded-[1.5rem] bg-white shadow-lg">
                      {logoUrl ? <ContentImage src={logoUrl} alt={brandName} fill className="object-cover" /> : null}
                    </div>
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-teal-800">Identity card</p>
                      <p className="mt-1 text-lg font-semibold text-slate-950">Profile presentation with stronger editorial hierarchy</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <section className="mt-10 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <RichContent html={descriptionHtml} className={`mx-auto max-w-none rounded-[2rem] p-6 ${experience.panelClass}`} />
